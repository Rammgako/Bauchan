...
class Daemon(object):
    ...
    def run(self):
        agent_pid = os.getpid()
        logging.info('Running Bauchan daemon with PID=%s' % agent_pid)
...
class NativeMessagingConfiguration(object):
    ...
    @staticmethod
    def get_path():
        return os.path.expanduser('~/Library/Application Support/Mozilla/NativeMessagingHosts/bauchan.json')
    ...
    def build():
        return dict(
            ...
            path=os.path.join(os.getcwd(), 'bauchan.py'),
            type='stdio',
            allowed_extensions=[
                'bauchan@example.org',
            ],
...
class LaunchctlManager(object):
    ...
    def source_path(self):
        return os.path.abspath(os.path.join('.', 'org.example.bauchan.agent.plist'))

    @property
    def target_path(self):
        return os.path.expanduser(os.path.join('~', 'Library', 'LaunchAgents', 'org.example.bauchan.agent.plist'))
...
class App(rumps.App):
    def __init__(self, launchctl_manager):
        super(App, self).__init__(name='Bauchan')
...
if __name__ == "__main__":
    common.configure_logging('bauchand')
    try:
        main()
    except Exception as e:
        logging.critical('Critical error', e)
    logging.info('Quit bauchand')
